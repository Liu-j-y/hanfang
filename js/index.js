window.onload = function() {
    ...
    function banner(parent, controller, mod) {
        ...
        for (var i = 0; i < controller.length; i++) {
            ...
            controller[i].onmouseover = function() {
                for (var j = 0; j < controller.length; j++) {
                    //将所有控制器改为默认样式
                    controller[j].className = 'dot-li';
                }
                //改变所选的控制器样式
                this.className = 'dot-li active';
                //将模型改为默认样式
                for (var x = 0; x < mod.length; x++) {
                    mod[x].className = 'img-li';
                }
                //改变所选控制器对应的模型的样式
                mod[this.index].className = 'img-li active';
            }
        }
        //绑定鼠标移入,暂停播放图片
        ...
        function play(num) {
            ...
            controller[num].className = 'dot-li active';
            for (var x = 0; x < mod.length; x++) {
                mod[x].className = 'img-li';
            }
            mod[num].className = 'img-li active';
        }
        //自动播放
        function autoPlay() {
